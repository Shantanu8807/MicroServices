...
import com.account.dto.AccountsMessageDto;
...
public class AccountServiceImpl implements IAccountsService {
	...
	private void sendCommunication(Accounts account, Customer customer) {
		AccountsMessageDto accMsgDto = new AccountsMessageDto(account.getAccountNumber(), customer.getName(),
				customer.getEmail(), customer.getMobileNumber());

		log.info("sending communication reqeust for the deaetisl ", accMsgDto);

		var result = streamBridge.send("sendCommunication-out-0", accMsgDto);

		log.info("communication request processed success fully");
	}
	...
	public boolean updateAccount(CustomerDto customerDto) {
		...
		if (accountsDto != null) {
			Accounts accounts = accountRepo.findById(accountsDto.getAccountNumber())
	...
	public boolean updateCommunicationSwitch(Long accountNumber) {
		...
		if (accountNumber != null) {
			Accounts accounts = accountRepo.findById(accountNumber)
					.orElseThrow(() -> new ResourceNotFoundException("No Account Found"));
